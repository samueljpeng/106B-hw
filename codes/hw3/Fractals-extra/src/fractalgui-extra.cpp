...
void FractalGUI::doCarpet() {
    ...
    int size = sizeField->getValueAsInteger();
    int order = orderField->getValueAsInteger();
    std::string statusText = "Sierpinski carpet: x=" + integerToString(x)
            + " y=" + integerToString(y)
            + " size=" + integerToString(size)
            + " order=" + integerToString(order);
    setStatusText(statusText + " ...");
    ...
    drawCarpet(*window, x, y, size, order);
...
void FractalGUI::initialize() {
    ...
    newFractalButton = createButton("New Fractal", "colored-rectangles-icon.png");
#ifdef TREE_ENABLED
    ...
    window->addToRegion(newFractalButton, "West");
#ifdef TREE_ENABLED
